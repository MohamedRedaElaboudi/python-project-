...
@plagiat_bp.route("/dashboard", methods=["GET"])
def dashboard_plagiat():
    ...
    for analysis, rapport, user, student_info in results:
        ...
        data.append({
            ...
            "level": student_level,
            "rapportName": rapport.filename if rapport else "Non li├®",
            ...
            "juryAssigned": analysis.jury_assigned or "Non assign├®"
        })
...
def get_analysis_details(analysis_id):
    ...
    if not analysis:
        return jsonify({"message": f"Analyse de plagiat (ID: {analysis_id}) non trouv├®e"}), 404
