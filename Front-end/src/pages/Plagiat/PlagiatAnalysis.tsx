...
export const PlagiatAnalysis: React.FC = () => {
  ...
  const fetchAnalysis = async (analysisId: string) => {
    ...
    try {
      const res = await axios.get<ApiResponse>(
        `http://127.0.0.1:8000/api/plagiat/analysis/${analysisId}`
      );
      setAnalysisData(res.data.analysis);
      console.log("Données reçues:", res.data.analysis);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const res = await axios.post(
        `http://127.0.0.1:8000/api/plagiat/generate-report/${id}`,
  ...
  const handleReanalyze = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(
        `http://127.0.0.1:8000/api/plagiat/analyze/${id}`,
        {}
      );
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} lg={mainGridSize}>
          <Grid container spacing={3}>
